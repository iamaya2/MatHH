classdef JSSP < handle
    ...
    methods (Static)
        ...
        function allInstances = loadInstanceSet(setID)
            ...
            %
            % Available datasets:
            %   E01                 - A set with 30 instances of size 15x15
            %   E02                 - A set with 30 instances of size 5x5
            %   KnownOptima         - A set with 24 instances of different
            %   sizes, ranging from 103*100 to 21338*1000 (broken)
            %   ORLibrary           - A set with 82 instances of different sizes
            %   from the OR-Library (broken)
            %   TailoredCombined    - A set with 240 instances of size 3x4
            %
            % Input:
            ...
            %
            % Output:
            %   allInstances - The set of JSSPInstance objects, given in a
            %   cell array
            %
            % See also: JSSPINSTANCE

            baseMatPath = '..\..\..\BaseInstances\JobShopScheduling\files\mat\Instances\';
            switch lower(setID)
                case 'e01'
                    load([baseMatPath 'E01\instanceDataset.mat'], 'allInstances')

                case 'e02'
                    load([baseMatPath 'E02\instanceDataset.mat'], 'allInstances')

                case 'knownoptima'
                    load([baseMatPath 'KnownOptima_Benchmark\instanceDataset.mat'], 'allInstances')
